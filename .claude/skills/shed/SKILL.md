...
2. Check `bin/ry-board.sh`. For each engine whose next step is known but not yet recorded, write it as the final line of `state/<id>.waybill.md` prefixed `NEXT:` so a fresh session can resume.
