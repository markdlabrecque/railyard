...
4. Replace the contents of `state/open-decisions.md` (do not append) with one line each for every item still awaiting the dispatcher's word — an unanswered decision, an ask discussed but never dispatched, or a review judgment a fresh session would otherwise have to re-derive. Carry the task id where there is one.
